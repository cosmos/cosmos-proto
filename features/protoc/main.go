...
func fieldGoType(g *generator.GeneratedFile, f *fileInfo, field *protogen.Field) (_ interface{}, pointer bool) {
	...
	if opt != nil {
		num, _ := unwrapOption(opt.String())
		// check if its an interface option, in this case we want to override any
		if num == acceptsInterfaceFieldOption {
			return field.GoIdent, false
		}
	}
	if field.Desc.IsWeak() {
		return "struct{}", false
