		...
		for _, message := range messages {
			for _, field := range message.Fields {
				...
				if opt != nil {
					...
					key, val := unwrapOption(opt.String())

					// check if its an interface option, in this case we want to override Anys and convert them to
					// the underlying interface specified by the option in the yaml config file
					if key == acceptsInterfaceFieldOption {
