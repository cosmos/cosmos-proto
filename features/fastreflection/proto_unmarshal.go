...
func (g *fastGenerator) genUnmarshalMethod() {
	...
	g.P(`return `, protoifacePkg.Ident("UnmarshalOutput"), "{Flags: input.Flags},", g.Ident("fmt", "Errorf"), `("proto: `, g.message.GoIdent.GoName, `: wiretype end group for non-group")`)
	g.P(`}`)
	g.P(`if fieldNum <= 0 {`)
	g.P(`return `, protoifacePkg.Ident("UnmarshalOutput"), "{Flags: input.Flags},", g.Ident("fmt", "Errorf"), `("proto: `, g.message.GoIdent.GoName, `: illegal tag %d (wire type %d)", fieldNum, wire)`)
	g.P(`}`)
	g.P(`switch fieldNum {`)
	...
	if len(g.message.Extensions) > 0 {
		...
		g.P(`return `, protoifacePkg.Ident("UnmarshalOutput"), "{Flags: input.Flags},", runtimePackage.Ident("ErrInvalidLength"))
		g.P(`}`)
		g.P(`if (iNdEx + skippy) > l {`)
		g.P(`return `, protoifacePkg.Ident("UnmarshalOutput"), "{Flags: input.Flags},", g.Ident("io", `ErrUnexpectedEOF`))
		g.P(`}`)
	...
	for _, field := range g.message.Fields {
		...
		g.P(`return `, protoifacePkg.Ident("UnmarshalOutput"), "{Flags: input.Flags},", `new(`, g.Ident(generator.ProtoPkg, "RequiredNotSetError"), `)`)
		g.P(`}`)
	}
	g.P()
	g.P(`if iNdEx > l {`)
	g.P(`return `, protoifacePkg.Ident("UnmarshalOutput"), "{Flags: input.Flags}, ", g.Ident("io", `ErrUnexpectedEOF`))
	g.P(`}`)
	g.P(`return `, protoifacePkg.Ident("UnmarshalOutput"), "{Flags: input.Flags}, ", `nil`)
	g.P(`}`)
}
...
func (g *fastGenerator) decodeVarint(varName string, typName string) {
	...
	g.P(`if shift >= 64 {`)
	g.P(`return `, protoifacePkg.Ident("UnmarshalOutput"), "{Flags: input.Flags}, ", runtimePackage.Ident("ErrIntOverflow"))
	g.P(`}`)
	g.P(`if iNdEx >= l {`)
	g.P(`return `, protoifacePkg.Ident("UnmarshalOutput"), "{Flags: input.Flags}, ", g.Ident("io", `ErrUnexpectedEOF`))
	g.P(`}`)
...
func (g *fastGenerator) unmarshalField(field *protogen.Field, message *protogen.Message, proto3 bool, required protoreflect.FieldNumbers) {
	...
	if field.Desc.IsList() && wireType != protowire.BytesType {
		...
		g.P(`}`)
		g.P(`} else {`)
		g.P(`return `, protoifacePkg.Ident("UnmarshalOutput"), "{Flags: input.Flags},", g.Ident("fmt", "Errorf"), `("proto: wrong wireType = %d for field `, errFieldname, `", wireType)`)
		g.P(`}`)
	} else {
		g.P(`if wireType != `, strconv.Itoa(int(wireType)), `{`)
		g.P(`return `, protoifacePkg.Ident("UnmarshalOutput"), "{Flags: input.Flags},", g.Ident("fmt", "Errorf"), `("proto: wrong wireType = %d for field `, errFieldname, `", wireType)`)
...
func (g *fastGenerator) decodeMessage(varName, buf string, message *protogen.Message) {
	local := g.IsLocalMessage(message)
	...
	if local {
		...
		g.P(`if unmarshal, ok := interface{}(`, varName, `).(interface{`)
		g.P(`Unmarshal([]byte) error`)
		g.P(`}); ok{`)
		g.P(`if err := unmarshal.Unmarshal(`, buf, `); err != nil {`)
		g.P(`return `, protoifacePkg.Ident("UnmarshalOutput"), "{Flags: input.Flags},", `err`)
		g.P(`}`)
		g.P(`} else {`)
		g.P(`if err := `, g.Ident(generator.ProtoPkg, "Unmarshal"), `(`, buf, `, `, varName, `); err != nil {`)
		g.P(`return `, protoifacePkg.Ident("UnmarshalOutput"), "{Flags: input.Flags},", `err`)
		g.P(`}`)
		g.P(`}`)
	}
