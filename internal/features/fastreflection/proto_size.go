...
func (g *fastGenerator) field(proto3 bool, field *protogen.Field, oneof bool) {
	...
	wireType := generator2.ProtoWireType(field.Desc.Kind())
	fieldNumber := field.Desc.Number()
	if packed {
		wireType = protowire.BytesType
	}
	key := generator2.KeySize(fieldNumber, wireType)
	...
	case protoreflect.MessageKind:
		if field.Desc.IsMap() {
			fieldKeySize := generator2.KeySize(field.Desc.Number(), generator2.ProtoWireType(field.Desc.Kind()))
			goTypeK, _ := g.FieldGoType(field.Message.Fields[0])
			goTypeV, ptr := g.FieldGoType(field.Message.Fields[1])
			if ptr {
				goTypeV = "*" + goTypeV
			}
			keyKeySize := generator2.KeySize(1, generator2.ProtoWireType(field.Message.Fields[0].Desc.Kind()))
			valueKeySize := generator2.KeySize(2, generator2.ProtoWireType(field.Message.Fields[1].Desc.Kind()))
