...
func (g *fastGenerator) genUnmarshalMethod() {
	...
	if len(g.message.Extensions) > 0 {
		...
		g.P(`}`)
		g.P(g.Ident(generator2.ProtoPkg, "AppendExtension"), `(m, int32(fieldNum), dAtA[iNdEx:iNdEx+skippy])`)
		g.P(`iNdEx += skippy`)
		g.P(`} else {`)
	}
	g.P(`iNdEx=preIndex`)
	...
	for _, field := range g.message.Fields {
		...
		g.P(`return `, protoifacePkg.Ident("UnmarshalOutput"), "{NoUnkeyedLiterals: input.NoUnkeyedLiterals, Flags: input.Flags},", `new(`, g.Ident(generator2.ProtoPkg, "RequiredNotSetError"), `)`)
		g.P(`}`)
	}
	g.P()
	g.P(`if iNdEx > l {`)
